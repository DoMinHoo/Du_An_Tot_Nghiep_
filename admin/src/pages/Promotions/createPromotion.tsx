...
import { ArrowLeftOutlined, GiftOutlined, PercentageOutlined, CalendarOutlined, UserOutlined } from "@ant-design/icons"
...
const CreatePromotion: React.FC = () => {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      message.error(error.response?.data?.error || "Tạo mã thất bại")
    },
  })
        ...
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ isActive: true }}>
                      ...
                      validator: (_, value) => {
                        ...
                        if (discountType === "percentage") {
                          if (value < 0 || value > 100)
                            return Promise.reject(new Error("Giảm phần trăm phải từ 0 đến 100"))
                        }
              ...
              <Col span={24}>
                ...
                >
                  <div>
                    ...
                    <Text type="secondary" style={{ fontSize: "12px" }}>
                      Mã khuyến mãi sẽ{" "}
                      {Form.useWatch("isActive", form) ? "được kích hoạt ngay" : "ở trạng thái tạm dừng"}
                    </Text>
                  </div>
          ...
          <Card style={{ borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
              ...
              >
                {mutation.isPending ? "Đang tạo..." : "Tạo mã khuyến mãi"}
              </Button>
            </div>
          </Card>
        </Form>
